        ...
        Auth.auth().signIn(withEmail: email, password: password) { user, error in
            if let error = error as NSError? {
                if error.code == AuthErrorCode.userNotFound.rawValue {
                    self.showAlert(titleString: "Error", messageString: "There is no user record corresponding to this credential. Please signup first!")
                } else if error.code == AuthErrorCode.wrongPassword.rawValue {
                    self.showAlert(titleString: "Error", messageString: "Password is incorrect. Please try again or reset password!")
                } else {
